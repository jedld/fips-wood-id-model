...
fused_trainset = torch.utils.data.ConcatDataset([train_dataset, train_dataset2])
...
classifier = ImageResNetTransferClassifier(num_classes=len(train_dataset.classes))
...
for epoch in range(30):  # loop over the dataset multiple times
    ...
    for i, data in enumerate(trainloader, 0):
        ...
        if (i % 10 == 9):
          print('[%d, %5d] loss: %.3f best: %.3f' %
                (epoch + 1, i + 1, running_loss / 10, best_accuracy))
          running_loss = 0.0
